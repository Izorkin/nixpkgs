...
, libtasn1, iptables, libgcrypt, yajl, pmutils, libcap_ng, libapparmor
...
in stdenv.mkDerivation rec {
  ...
  preConfigure = let
    ...
    '';
  in ''
    PATH=${lib.makeBinPath ([ dnsmasq ] ++ optionals stdenv.isLinux [ iproute iptables lvm2 systemd numad ] ++ optionals enableIscsi [ openiscsi ])}:$PATH
  ...
  postInstall = let
    binPath = [ iptables iproute pmutils numad numactl bridge-utils dmidecode dnsmasq ] ++ optionals enableIscsi [ openiscsi ];
  in ''
